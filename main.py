...
def transactions():
    ...
    print(dates[0].strip())
    print("Read date: " + time.strptime(str(dates[0].strip()), "%m/%d/%Y"))
    start = time.strptime(str(dates[0].strip()), "%m/%d/%Y").isoformat()
    end = time.strptime(str(dates[1].strip()), "%m/%d/%Y").isoformat()
    print("Start: " + start)
    print("End " + end)
    plaid = Plaid(PLAID_ID, PLAID_KEY, key)
